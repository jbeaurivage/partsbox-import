...
with open(backup_path, "r") as f:
    backup_data = json.load(f).get("parts")

# Build a mapping from storage name to old ID
backup_name_to_id = {
    loc["part/name"]: loc["part/id"]
    for loc in backup_data
    if "part/name" in loc and "part/id" in loc
}
...
current_names = set(loc.get("part/name") for loc in current_locations if "part/name" in loc)

# Build mapping: old_id -> new_id for matching storage names
id_mapping = []
for loc in current_locations:
    name = loc.get("part/name")
    new_id = loc.get("part/id")
...
if missed_in_new:
    print("WARNING: The following parts are in the backup but not in the current data:")
    for name in missed_in_new:
        print(f"  - {name}")

if missed_in_old:
    print("WARNING: The following parts are in the current data but not in the backup:")
